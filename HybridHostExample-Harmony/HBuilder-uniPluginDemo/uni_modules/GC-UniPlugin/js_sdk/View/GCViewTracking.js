...
class PageMonitor {
	...
	startTracking(app) {
		...
		try {
			...
			console.log('[FTLog] View tracking plugin initialized successfully', " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:52");
			// #endif
		} catch (error) {
			console.error('[FTLog] View tracking plugin initialization failed:', error, " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:55");
			this.initialized = false;
		}
	}
	installPageHooks(app) {
		...
		if (app && typeof app.mixin === 'function') {
			app.mixin(mixin);
			console.log('[FTLog] View tracking page hooks installed through app.mixin', " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:80");
			return true;
		}
		...
		if (typeof Vue !== 'undefined' && Vue && typeof Vue.mixin === 'function') {
			Vue.mixin(mixin);
			console.log('[FTLog] View tracking page hooks installed through Vue.mixin', " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:87");
			return true;
		}
		// #endif

		console.warn('[FTLog] View tracking page hooks were not installed, falling back to router success timing', " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:92");
		return false;
	}
	checkInitialPage() {
		if (!this.initialized) return;
		...
		console.log('[FTLog] Existing page detected without a load lifecycle:' + pagePath, " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:109");
		...
		try {
			const addAppListener = (event, callback) => {
				try {
					...
					console.warn(`[FTLog] Unsupported event listener type: ${event}`, " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:142");
					return false;
				} catch (e) {
					console.error(`[FTLog] Failed to add event listener ${event}:`, e, " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:145");
					return false;
				}
			};
			...
			console.log('[FTLog] watchAppLifecycle internal logic executed successfully', " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:160");
		} catch (error) {
			console.error('[FTLog] Error executing watchAppLifecycle method:', error, " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:162");
	...
	handleAppShow() {
		...
		if (!pagePath) return;
		this.currentPage = pagePath;
		console.log('[FTLog] App display detected:' + pagePath, " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:175");
		if (!currentPage || !this.pendingPageLoads.has(currentPage.$vm)) {
			this.activateView(pagePath);
		}
	}
	...
	handleAppHide() {
		console.log('[FTLog] App hiding detected', " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:182");
	...
	rumStartView(){
		...
		console.log('[FTLog] this.currentPage:'+this.currentPage, " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:250");
		if (this.currentPage) {
	...
	activateView(pagePath) {
		...
		console.log('[FTLog] startView：' + view_name, " at uni_modules/GC-UniPlugin/js_sdk/View/GCViewTracking.js:392");
		this.rum.startView({
			'viewName': view_name,
			'property': {
