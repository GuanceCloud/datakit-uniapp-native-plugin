...
export const gcWatchRouter = {
	...
	onShow: function() {
		...
		if (pages.length > 0) {
			...
			console.log("onShow", pageInstance, " at uni_modules/GC-UniPlugin/js_sdk/View/GCWatchRouter.js:28")
	...
	methods: {
		...
		rumStopOldStartNew(addListener = true) {
			if (this.currentPage) {
				console.log("stopView", " at uni_modules/GC-UniPlugin/js_sdk/View/GCWatchRouter.js:95")
				rum.stopView(null)
		...
		addPopGestureEventListener() {
			...
			if (pages.length > 0) {
				...
				let gc = this;
				console.log('addEventListener:' + object, " at uni_modules/GC-UniPlugin/js_sdk/View/GCWatchRouter.js:137")
				object.addEventListener('popGesture', gc.eventListenerPopGesture)
				let orionclose = object.onclose
				object.onclose = function(result) {
					console.log('onclose', " at uni_modules/GC-UniPlugin/js_sdk/View/GCWatchRouter.js:141")
		...
		eventListenerPopGesture(e) {
			...
			if (progress == 100) {
				console.log('popGesture', e, " at uni_modules/GC-UniPlugin/js_sdk/View/GCWatchRouter.js:152")
				this.navigateBack()
			}
		},
		parseUrl(url) {
