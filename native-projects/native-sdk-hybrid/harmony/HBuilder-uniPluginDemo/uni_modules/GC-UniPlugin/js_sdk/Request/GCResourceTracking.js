...
function completeResource(request, response) {
	try {
		...
		console.log('[GC-UniPlugin] Harmony uni.request resource completed:', response.statusCode, request.url, request.key, " at uni_modules/GC-UniPlugin/js_sdk/Request/GCResourceTracking.js:51");
	} catch (error) {
		console.error('[GC-UniPlugin] Harmony uni.request tracking success failed:', error, " at uni_modules/GC-UniPlugin/js_sdk/Request/GCResourceTracking.js:53");
	}
}
...
function completeResourceError(request, error) {
	try {
		...
		console.warn('[GC-UniPlugin] Harmony uni.request resource failed:', errorMessage, request.url, request.key, " at uni_modules/GC-UniPlugin/js_sdk/Request/GCResourceTracking.js:77");
	} catch (trackingError) {
		console.error('[GC-UniPlugin] Harmony uni.request tracking fail failed:', trackingError, " at uni_modules/GC-UniPlugin/js_sdk/Request/GCResourceTracking.js:79");
	}
}
...
export const gcHarmonyNetworkTracking = {
	startTracking() {
		...
		if (interceptorInstalled || !isHarmonyPlatform() ||
			...
			}, " at uni_modules/GC-UniPlugin/js_sdk/Request/GCResourceTracking.js:100");
			return false;
		}
		...
		uni.addInterceptor('request', {
			invoke(options) {
				try {
					...
					console.log('[GC-UniPlugin] Harmony uni.request start:', options.method || 'GET', options.url, key, " at uni_modules/GC-UniPlugin/js_sdk/Request/GCResourceTracking.js:128");
					...
					console.error('[GC-UniPlugin] Harmony uni.request tracking invoke failed:', error, " at uni_modules/GC-UniPlugin/js_sdk/Request/GCResourceTracking.js:150");
				}
				return options;
			}
		});
		interceptorInstalled = true;
		console.log('[GC-UniPlugin] Harmony uni.request tracker installed:', platform, " at uni_modules/GC-UniPlugin/js_sdk/Request/GCResourceTracking.js:156");
		return true;
		// #endif
		return false;
	},
